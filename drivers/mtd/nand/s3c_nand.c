...
{
        void __iomem *regs = s3c_nand.regs;
        while (!(readl(regs + S3C_NFSTAT) & S3C_NFSTAT_BUSY))
        {
            printk("NandFlash is busying.........\n");
        }
        return 1;
}
...
int s3c_nand_correct_data_8bit(struct mtd_info *mtd, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
{
	...
	switch (err_type) {
        case 12: /* Uncorrectable */
                printk("s3c-nand: 12 bit(s) error detected ECC uncorrectable error detected\n");
                ret = -1;
                break;
        case 11: /* Uncorrectable */
                printk("s3c-nand: 11 bit(s) error detected ECC uncorrectable error detected\n");
                ret = -1;
                break;
        case 10: /* Uncorrectable */
                printk("s3c-nand: 10 bit(s) error detected ECC uncorrectable error detected\n");
                ret = -1;
                break;
	case 9: /* Uncorrectable */
                printk("s3c-nand: 9 bit(s) error detected  ECC uncorrectable error detected\n");
		ret = -1;
		break;
